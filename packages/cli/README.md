...
The command emits one canonical result-v1 JSON object. The default gate blocks
`error` and `high` findings. Exit codes are `0` for pass or advisory, `1` for a
blocked gate, `2` for configuration or input errors, and `3` for internal
failures.
